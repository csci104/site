...
- It uses Manhattan distance ( $$\abs{goal_x - curr_x} - \abs{goal_y - curr_y}$$ ) to the target node for the heuristic (the h-value) and distance travelled from the source along actual edges (through the predecessor nodes for the g-value)

...
**NOTE:** There is no specific formatting requirement for the output of your `counting` program. This portion of the assignment will be graded with human eyes, so you just need to make sure you record and report the time for inserting/updating all the words in the data structure, and output the number of occurrences for all words in the input text. Your output should be readable (again, human eyes are grading this), and should match up with whatever you have reported in the `README.md` (which is described below.) 
